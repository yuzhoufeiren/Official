  ...
  methods: {
    ...
    updateinfo:function(){
      return this.$axios.get("/api/article/test").then(res=>{
        ...
        let temp2;
        for (let i = 0; i < res.data.data.length; i++) {
              temp1 = res.data.data[i].content1.split(",");//纵向
              temp2 = res.data.data[i].content2.split(",");//横向
          let temp3 = res.data.data[i].content3.split(",");//风速
          let temp4 = res.data.data[i].content4; //风向
          this.content.push(temp1[temp1.length - 1].replace("]", ""));
          this.content.push(temp2[temp2.length - 1].replace("]", ""));
        ...
        let bs = this.calcBridgeStatus(temp1[temp1.length - 1].replace("]", ""),parseInt(temp2[temp2.length - 1].replace("]", "")));
        this.polyline.strokeColor = this.getbridecolor(bs);
      })
    },
    calcBridgeStatus(temp1,temp2) {
      let bs;
      if(temp2<=15.7||temp2>20.7||temp1<37) {
        bs = 1;}
      else if(temp2<=17.5||temp2>=18.6) {
        bs = 2;
      } else {
        bs = 3;
      }
    ...
    getBridgeStatus: function() {
      this.$axios.get('/api/article/test').then(res => {
        ...
        this.bridgeStatus = this.calcBridgeStatus(temp1[temp1.length - 1].replace("]", ""), parseInt(temp2[temp2.length - 1].replace("]", "")));
      })
    },
    //显示信息框
